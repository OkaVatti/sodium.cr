...
if [ "${LIBSODIUM_INSTALL:-}" != "1" ]; then
  [ -n "${SODIUM_BUILD_VERBOSE:-}" ] && echo "Skipping libsodium build." 1>&2
  exit 0
fi
...
if [ ! -f "$LIBSODIUM_INSTALL_PATH/include/sodium.h" ]; then
  ...
  [ -n "${SODIUM_BUILD_VERBOSE:-}" ] && echo "Custom libsodium built at $LIBSODIUM_INSTALL_PATH" 1>&2
else
  [ -n "${SODIUM_BUILD_VERBOSE:-}" ] && echo "Custom libsodium already exists at $LIBSODIUM_INSTALL_PATH" 1>&2
fi
